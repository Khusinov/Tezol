...
import uz.khusinov.marjonamarketcourier2.utills.price
...
class ProductDetailsBottomSheet : BottomSheetDialogFragment(R.layout.bottom_sheet_product_details) {
    ...
    private fun setupUi() = with(binding) {
        if (product.img != null)
            Picasso.get().load(product.img).into(productImage)
        productName.text = product.name
        productDescription.text = product.more
        productPrice.text = price(product.price.toInt())
